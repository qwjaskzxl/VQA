...
代码说明：
- tools：
    - create_dictionary.py：创建q中所有词的词典、加载glove weights
    - compute_softscore.py：对a预处理、挑选出现>=9词的a、建立a2i/i2s.pkl、计算每个q对应a的sc
        - 话说...预处理为啥不直接只保留eng、num、space呢？(不过要注意把都转化为阿拉伯数字)
    - detection_features_converter.py：
数据说明：
- train_ids.pkl/val_ids.pkl：a set, len:82783/40504
- dictionary.pkl：[dict(word:idx), list(word)], len:19901