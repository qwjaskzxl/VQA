...
comma_strip = re.compile("(\d)(\,)(\d)") #在处理标点时用，因为这是小数点的情况？
punct = [';', r"/", '[', ']', '"', '{', '}',
                '(', ')', '=', '+', '\\', '_', '-',
                '>', '<', '@', '`', ',', '?', '!']
...
def get_score(occurences):
    # 算出每个问题的每个回答对应的acc_score，这样模型给出y_pred之后直接查表就知道acc_score了
    if occurences == 0:
        return 0
    elif occurences == 1:
        return 0.3
...
def process_punctuation(inText):
    ...
    for p in punct:
        if (p + ' ' in inText or ' ' + p in inText) or (re.search(comma_strip, inText) != None):
            outText = outText.replace(p, '') #如果有标点就去掉
        else:
            outText = outText.replace(p, ' ') #去掉的都是"-"？？？
    outText = period_strip.sub("", outText, re.UNICODE) #主要去掉xxx.com/xxx.org中的"."
    return outText
...
def create_ans2label(occurence, name, cache_root='data/cache'):
    ...
    utils.create_dir(cache_root) #mkdir
...
def compute_target(answers_dset, ans2label, name, cache_root='data/cache'):
    ...
    for ans_entry in answers_dset:
        # 算出每个问题的每个回答对应的acc_score，这样模型给出y_pred之后直接查表就知道acc_score了
        answers = ans_entry['answers'] #dict{ans:,confidence:,ans_id:}
        answer_count = {} #dict{ans:count}
        for answer in answers: #该问题的每个回答
            answer_ = answer['answer'] #an answer string
            answer_count[answer_] = answer_count.get(answer_, 0) + 1 #返回key的value，若key不存在则返回0
...
if __name__ == '__main__':
    ...
    occurence = filter_answers(answers, 9) ##dict{word:{q_ids}}  筛出出现>=9的ans以及对应的q_id
    ans2label = create_ans2label(occurence, 'trainval') #创建a2i和i2a的.pkl
